"""
Создание модели из исходного ноутбука.
"""
import warnings
...
import constants as add

# Создадим функцию для расчета времени выполнения операций
...
# Импорт необходимых библиотек для подготовки и оценки модели
...
print('Все библиотеки загружены. Время выполнения..', time_it())

# Исходная информация о модели
RANDOM_SEED = 0
VERSION = 'Not Detected :-)'
AUTHOR = 'Alexander Vinogradov'
NAME = "Prediction of the user's target action"
DESCRIPTION = 'Модель по предсказанию совершения пользователем одного из целевых действий на сайте сервиса СберАвтоподписка.'

# Пропишем пути к данным и к папке с моделями
DATA_FOLDER = Path('.', 'data')
SESSIONS_FILENAME = 'ga_sessions.csv'
HITS_FILENAME = 'ga_hits.csv'
MODELS_FOLDER = Path('.', 'model')
...
def create_model(
    ...
    model = _get_model()
    print('Загружены данные в датасет. Время выполнения...', time_it())
    # Метаданные и метрики модели
    metadata = _get_metadata(model, X, y)
    print('Заполнены метаданные. Время выполнения...', time_it())
    # Обучение модели на всех данных
    model.fit(X, y)
    print('Модель обучена. Время выполнения....', time_it())
    # Сохранение модели с метаданными
    _save_model(model, metadata, models_folder or MODELS_FOLDER)
    print('Модель сохранена с метаданными:', metadata, 'в папке', models_folder, ' Время выполнения.....', time_it())
...
def _load_data(folder: PathLike) -> Tuple[pd.DataFrame, pd.Series]:
    ...
    print('Загрузка данных из', sessions_file, 'прошла успешно. Время загрузки...', time_it())
    hits = pd.read_csv(hits_file, usecols=['session_id', 'event_action'], low_memory=False)
    print('Загрузка данных из', hits_file, 'прошла успешно. Время загрузки...', time_it())
    ...
    print('Целевые переменные подготовлены. Время выполения...', time_it())
    return sessions, target.fillna(0.0)


def _get_model() -> BaseEstimator:
    """Возвращает модель с гиперпараметрами, найденными в ноутбуке
    с исследованием модели.
    """
    print('Началось создание модели. Время выполнения ....', time_it())
...
def _fill_missings(data: pd.DataFrame) -> pd.DataFrame:
    ...
    * значением '(nan)' во всех остальных случаях.
    ...
    print('Заполнены пропущенные значения для `device_screen_resolution`. Время выполнения...', time_it())
...
def _distance_category(distance: float) -> str:
    ...
    if distance == -1:
        ...
        return '>= 3000 km'
    print('Созданы категории расстояния до Москвы. Время выполнения....', time_it())
...
def _get_metadata(model, X, y) -> _metadata_type:
    ...
    print(" ")
    print('Оценка модели. Время выполнения...', time_it())
    return {
        'name': NAME,
...
def _save_model(
    ...
    """Сохраняем модель в папку с моделями."""
    ...
    print('Модель успешно сохранена!!! Время выполнения...', time_it())
    print(filename)
